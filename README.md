...
## Validate Header CSV
El objetivo del flujo consiste en leer un csv que contiene un header y validar que la cantidad de columnas que contiene sea la deseada. Para el ejemplo, se lee el archivo llamado "username.csv" el cual contiene 6 columnas. Como mi valor de columnas esperadas (6 en nuestro caso) será siempre igual, voy a validar que siempre vengan 6 columnas dado que el mismo es un archivo que genera un usuario manualmente y puede cometer errores en su confeccion. En caso de que haya errores loguea el mensaje de error, caso contrario vuelve a escribir el mismo archivo con el mismo nombre.
